-- copy from: https://github.com/input-output-hk/typed-protocols/blob/main/typed-protocols-examples/src/Network/TypedProtocol/Channel.hs
{-# LANGUAGE FlexibleContexts #-}
{-# LANGUAGE NamedFieldPuns #-}
{-# LANGUAGE RankNTypes #-}
{-# LANGUAGE ScopedTypeVariables #-}

module Process.Effect.Network.Channel
  ( Channel (..),
    hoistChannel,
    isoKleisliChannel,
    fixedInputChannel,
...
isoKleisliChannel ::
  ...
  Channel m a ->
  Channel m b
isoKleisliChannel f finv Channel {send, recv} =
  Channel
    { send = finv >=> send,
    ...
    queuesAsChannel bufferRead bufferWrite =
      ...
      where
        send x = atomically $ do
          ...
          if full
            then error failureMsg
            else writeTBQueue bufferWrite x
        recv = atomically (Just <$> readTBQueue bufferRead)

    failureMsg =
      "createPipelineTestChannels: "
        ++ "maximum pipeline depth exceeded: "
        ++ show sz
